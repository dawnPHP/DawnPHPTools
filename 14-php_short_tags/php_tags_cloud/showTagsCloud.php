...
function getCloud( $data = array(), $minFontSize = 12, $maxFontSize = 50 ){
	...
	foreach( $data as $tag => $count )
	{
		...
		$cloudTags[] = '<a style="font-size: ' . floor( $size ) . 'px'
					. '" href="index.php?tag='.stripslashes( $tag ).'" title="\'' . $tag .
...
$sql="select tag, count(t_id) as num from tags a,article_tags b where a.id=b.t_id and a.u_id='1' group by tag;";//改变用户名id
