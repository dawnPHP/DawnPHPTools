...
class UploadFile{
	...
	*/
	function upload_to($dir_name="upload/"){
		//判断文件是否合法
		$arr=$this->isValid($this->restricts);
		if( $arr[0]==0 )
			return $this->isValid($this->restricts);
		...
		$file=$this->file;//获取文件
		if (file_exists($dir_name . $file["name"])){
			return array(0, $file["name"] . " already exists. ");
		}else{
			$to=iconv("UTF-8","GB2312//IGNORE",$file["name"]);
			//网页是utf8, 而win7文件系统是gbk
			move_uploaded_file($file["tmp_name"],
			$dir_name . $to);//上传文件
			return array(1, $dir_name . $file['name']);
		}
	}
